    ...
    async manageMovieInfo() {
      if (
        ...
        if (json.state === 200) {
          ...
          if (json.data) {
            if (json.data.img) this.movieInfo.img = 'https://mokespace.cn/weimai/upFile/' + json.data.img
            if (json.data.videoImg) this.movieInfo.videoImg = 'https://mokespace.cn/weimai/upFile/' + json.data.videoImg
            if (json.data.video) this.movieInfo.videoUrl = 'https://mokespace.cn/weimai/upFile/' + json.data.video
          }
          //判断是编辑还是添加
          if (this.dialogTitle === '编辑电影信息') {
